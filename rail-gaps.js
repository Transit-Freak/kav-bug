 ...
 * באחד ולא בשני.
 *
 * *** מושבת (19.7.2026) *** — פנינו לרכבת ישראל בבקשה לאשר שימוש ב-API הציבורי
 * שלהם לצורך הכלי הזה, והם סירבו (הנימוק: שמירה על יציבות וזמינות המערכות עבור
 * ציבור הנוסעים). בהתאם לכך, הסקריפט הזה כבר לא מופעל — לא אוטומטית (הוסר
 * מ-weekly-scan.yml) ולא ידנית. הוא נשמר כאן לצורכי שקיפות/היסטוריה בלבד; אין
 * להריץ אותו שוב. פרטים והודעה לציבור: rail.html.
 *
 * שימוש (היסטורי):  node rail-gaps.js <path-to-gtfs.zip> [out-prefix]
...
const ALIAS = {
  ...
  "מרכזית המפרץ": "מרכזית המפרץ קו החוף", "נתבג": "נתב ג", "פתח תקווה סגולה": "סגולה",
  "פתח תקווה קרית אריה": "קרית אריה", "קרית מלאכי יואב": "קרית מלאכי", "קרית ספיר נתניה": "נתניה קרית ספיר",
  "ראשון לציון הראשונים": "ראשונים", "ראשון לציון משה דיין": "רשל צ משה דיין", "שער צומת חולון": "צומת חולון",
  "תל אביב אוניברסיטה": "תא אוניברסיטה", "תל אביב האוניברסיטה אקספו": "תא אוניברסיטה", "תל אביב השלום": "השלום",
  "תל אביב סבידור מרכז": "תל אביב מרכז", "עפולה צפון": "עפולה", "מגדל העמק כפר ברוך": "כפר ברוך",
  "יקנעם כפר יהושע": "כפר יהושע",
};
const norm = (s) => (s || "").replace(/קריית/g, "קרית").replace(/[''"׳״\-\/()]/g, " ").replace(/\s+/g, " ").trim();
const canon = (s) => { const n = norm(s); return ALIAS[n] || n; };

// תיקוני-קוד ידניים: תחנות שה-API החי של רכבת ישראל מזהה בקוד-פנימי שאינו
// מופיע כלל במאגר "רכבת כבדה - תחנות נוסעים" של data.gov.il (78 תחנות בלבד).
// אומת ישירות מול ה-API (רכבת 824, ראש העין צפון->חדרה מזרח: stopStations
// כולל גם 4310=טירה-כוכב יאיר וגם 4300, בעוד ה-GTFS מציג באותה נקודה בדיוק
// "שומרון - טייבה") ועקבי על פני 56 רכבות שונות — בלי שום שם אחר מופיע אי-פעם
// לצד קוד 4300. כנראה קוד-פנימי חדש יותר לתחנה שב-data.gov.il רשומה תחת
// ASSET_NO=39 (מספר ישן יותר). לא ניתן לאמת מול data.gov.il עצמו כי הקוד
// 4300 פשוט לא קיים שם בכלל.
const MANUAL_CODE_NAME = { "4300": "שומרון - טייבה" };
...
(async () => {
  ...
  // מיפוי-שם<-קוד (לשאילתת fromStation/toStation) נבנה *לפני* שמוסיפים את
  // תיקוני-הקוד הידניים — הקוד הידני (4300) אומת רק כמזהה שמופיע בתוך
  // stopStations של תשובת ה-API, לא כקוד תקף לשאילתה עצמה (fromStation/
  // toStation); אסור שהוא "יגנוב" את הכיוון ההפוך מהקוד המקורי שכן עובד (39).
  const nameToCode = {}; for (const [code, nm] of Object.entries(codeName)) nameToCode[canon(nm)] = code;
  Object.assign(codeName, MANUAL_CODE_NAME); // רק אחרי בניית nameToCode

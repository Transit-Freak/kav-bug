...
(function () {
  ...
  // שער ארטיפקט-הצמדה (כיוון 1): אם ההצמדה המונוטונית של תחנה "נתקעה" מוקדם על
  // חצייה-עצמית של ה-shape (ה-along שלה קטן בהרבה מהנקודה הקרובה-באמת,
  // gnearAlong), המקטע סביבה מנופח לכל הלולאה ואינו אמין. סף הפער (בק"מ) שמעליו
  // מדלגים על המקטע.
  const SNAP_STUCK_KM = 0.3;
  // שער ארטיפקט-הצמדה (כיוון 2): הכיוון ההפוך (along גדול בהרבה מ-gnearAlong)
  // אינו ניתן לפסילה לפי גודל-הפער בלבד — קווים תקינים לגמרי (למשל shape שעובר
  // פעמיים ליד אותו רחוב) מראים אותו פער בלי שום בעיית-מדידה. הבדיקה האמינה כאן
  // ישירה: המרחק (בק"מ) בין נקודת-ההטלה (proj — הנקודה שממנה בפועל נמדד המקטע)
  // למיקום *האמיתי* של התחנה (מ-GTFS). סף המרחק שמעליו מדלגים על המקטע.
  const SNAP_PROJ_GAP_KM = 0.1;
  ...
  // שער "רק לפי רכב" (car-only candidate): מקטע שלא נמצא לו קו-ייחוס תקף
  // (אף קו אחר לא עוצר גם ב-X וגם ב-Y, או שכולם מתפתלים באותו מרחק) — אין
  // הוכחת-קו כלל, ולכן הגלאי הרגיל לעולם לא ייצור עליו issue. אבל המרחק
  // המצויר עשוי בכל זאת להיות הרבה יותר גדול מהקו-האווירי בין שתי התחנות —
  // מקטעים כאלה נאספים בנפרד כמועמדים לבדיקה מול ניווט-רכב אובייקטיבי
  // (OSRM, ב-scan-country.js). זו *לא* הכרעה עצמאית: התוצאה תלויה בבדיקה
  // חיצונית שאינה יודעת על נתיבי-תח"צ, ולכן תמיד קטגוריה נפרדת מ"אמיתי",
  // לא תחליף לה. הסף כאן שמרני-יחסית (פי 3 מהקו-האווירי, לפחות 250 מ' בזבוז
  // מוחלט) כדי לשמור את מספר המועמדים הארצי בטווח שאפשר לבדוק בזמן סביר.
  const CAR_ONLY_RATIO = 2.5;
  const CAR_ONLY_MIN_KM = 0.25;
  ...
  // זיהוי ישיר של הלוך-חזור/רטרייס בתוך מקטע יחיד: לכל נקודה, האם יש נקודה
  // *מוקדמת יותר* (פער-אינדקס ≥3, כדי לא לתפוס עקומה רגילה) שנמצאת קרוב אליה
  // (≤25מ')? אם חלק ניכר מהנקודות "חוזרות" על נקודה מוקדמת — זו לולאת-גישה
  // לתחנת-קצה בדרך-ללא-מוצא (הקו הולך ואז חוזר כמעט באותו נתיב), לא עיקוף.
  // עדיף על forwardKm/behind (שמודד נסיגה מול *ציר* המוצא-יעד בלבד): כשכיוון
  // ה"פנייה חזרה" מתלכד בקירוב עם כיוון המוצא-יעד הכללי, ה-t לא יורד מספיק
  // ו-behind נשאר false למרות רטרייס מובהק (נבדק בפועל: קו 45 "עלמה 1"→"עלמה
  // 2" — 27% מהנקודות חוזרות, behind=false). בדיקה ישירה על הגאומטריה עצמה,
  // בלי תלות בכיוון, תופסת את זה תמיד.
  function selfOverlapFrac(geom) {
    if (!geom || geom.length < 8) return 0;
    const ky = 110.57;
    let close = 0;
    for (let i = 3; i < geom.length; i++) {
      const kx = 111.32 * Math.cos(geom[i][0] * Math.PI / 180);
      let minD2 = Infinity;
      for (let j = 0; j < i - 2; j++) {
        const dy = (geom[i][0] - geom[j][0]) * ky, dx = (geom[i][1] - geom[j][1]) * kx;
        const d2 = dy * dy + dx * dx;
        if (d2 < minD2) minD2 = d2;
      }
      if (minD2 < 0.000625) close++; // 0.025² ק"מ² = 25מ'
    }
    return close / (geom.length - 3);
  }
  ...
  function detectCrossRef(lines) {
    ...
    for (const L of lines) {
      ...
      if (L.hasShape) for (let i = 0; i < L.stops.length - 1; i++) {
        ...
        // שער ארטיפקט-הצמדה. שני כיוונים, שני מבחנים — לא ניתן לאחד לבדיקה
        // סימטרית אחת (נבדק אמפירית על סריקה ארצית מלאה, ראה השוואה למטה):
        // (1) "נתקעה מוקדם" (קו 90 בצפת: "קניון/הגליל" נתקעה ב-along 5.0 במקום
        //     10.5) — proj עצמו יוצא קרוב לתחנה (חלק מלולאה שעוברת ליד פעמיים),
        //     כך שרק גודל-הפער מול gnearAlong (לכיוון הזה) חושף את התקלה.
        // (2) "עברה רחוק מדי" (קווים 34/36 ב"דליה", קו 97 ב"זמיר") — לכיוון ההפוך
        //     גודל-הפער מול gnearAlong *אינו* אמין: קווים תקינים לגמרי (62/29/
        //     29א/54) מראים פער-gnearAlong גדול באותו כיוון בלי שום בעיה בפועל.
        //     המבחן האמין כאן הוא ישיר — המרחק (ק"מ) בין proj (הנקודה שממנה
        //     בפועל נמדד המקטע) למיקום *האמיתי* של התחנה (מ-GTFS): תקלות אמיתיות
        //     יוצאות ≥128מ', מקטעים תקינים ≤39מ' — הפרדה נקייה, סף 100מ' באמצע.
        // מדלגים אם אחד משני המבחנים נופל.
        {
        ...
        // שער behind/רטרייס: מקטעים שחוזרים-לאחור מעבר לתחנת-המוצא (forwardKm),
        // *או* שהגאומטריה שלהם עצמה חוזרת-על-עצמה (selfOverlapFrac) — גישה
        // לתחנת-קצה בדרך-ללא-מוצא (כמו בית-חולים/מתחם), לא עיקוף-צד — מסוננים
        // החוצה כאן, *לפני* שהם מגיעים בכלל ל-OSRM. שני המבחנים ביחד, לא רק
        // forwardKm/behind לבדו: כשכיוון "הפנייה-חזרה" מתלכד עם כיוון המוצא-
        // יעד הכללי, ה-t לא יורד מספיק ו-behind נשאר false למרות רטרייס מובהק
        // (נבדק בפועל: קו 45 "עלמה 1"→"עלמה 2" — 27% מהנקודות חוזרות על נקודה
        // מוקדמת, behind=false — הבדיקה הישירה על הגאומטריה תפסה את זה, ולפני
        // התיקון הזה זה עבר לדוח בטעות). נבדק על הדוח הארצי אחרי הוספת המבחן
        // הישיר: עוד כ-56% מהמקטעים שנותרו הראו רטרייס שה-behind-הישן פספס.
        {
          const straightKm = haversine(X, Y);
          const isBehind = !!(fwd && fwd.behind) || selfOverlapFrac(L._geom && L._geom[i]) > 0.15;
      ...
      // מסירים מועמדי-רכב שקיבלו בכל זאת הוכחת-קו רגילה (found) — מיותר לבדוק
      // אותם שוב מול רכב, יש כבר הכרעה חזקה יותר.
      const foundIdx = new Set(found.map((f) => f.i));
      L._carCands = carCands.filter((c) => !foundIdx.has(c.i));
      finalizeLine(L);
    }
  }

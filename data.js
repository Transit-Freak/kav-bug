...
(function () {
  ...
  function snapStopsToShape(stops, shape) {
    ...
    const posOut = pos.map((P, i) => ({ seg: P.seg, t: P.t, along: P.along, proj: projOf(P), gnear: gnear[i] }));
  ...
  const SELFLOOP_APPROACH_DEG = 60; // קו-ייחוס שמגיע לתחנה הראשונה מכיוון שונה ביותר
                                    // מ-60° אינו הוכחה תקפה: הלולאה נובעת מציר-כניסה
                                    // אחר (תמרון היפוך), לא מבזבוז שאפשר לחסוך.
    ...
    for (const L of lines) {
      ...
      for (let i = 0; i < L._geom.length; i++) {
        ...
        for (const O of (servedBy.get(from.id) || [])) {
          ...
          if (!pair) continue;
          // שער ציר-כניסה: קו-הייחוס חייב להגיע לתחנה הראשונה במקטע (from) מאותו
          // כיוון כמו הקו הנבדק. אם הוא נכנס מצד אחר (סטיית-זווית >60°), הלולאה של
          // הקו הנבדק היא תמרון-היפוך הנובע מכיוון-ההגעה שלו — לא בזבוז שאפשר לחסוך
          // ע"י "ליישר". (כך לא מושווה סיבוב-הכרחי לקו שמגיע מרחוב אחר; קווים שחולקים
          // את אותו ציר-כניסה — סיבוב מיותר אמיתי — נשמרים, שכן הזווית ביניהם ~0°.)
          const angSL = approachAngleDiffAt(L, i, O, pair.bi);
          if (angSL != null && angSL > SELFLOOP_APPROACH_DEG) continue;

...
const CHANGELOG = [
  { version: "4.0", date: "14.6.2026", items: [
    "תצוגת \"כל הארץ\" — דוח מוכן של כל העיקופים בכל קווי התחבורה בישראל, נטען מיד גם בטלפון (בלי להעלות קובץ). נסרק מראש ומתעדכן אוטומטית אחת לשבוע מהנתונים הרשמיים של משרד התחבורה. אפשר לסנן, לחפש, וללחוץ על כל שורה כדי לראות את הקו על המפה — כל מסלול הקו בכחול, המקטע הבעייתי בכתום (עם חצי כיוון-נסיעה, מצויר מעל קו-ההשוואה הירוק), והמפה ממוקדת ישירות באזור העיקוף.",
    "חישוב בזבוז: לכל עיקוף מוצג כמה ק\"מ הוא מבזבז ביום עמוס (העיקוף × מספר הנסיעות ביום). בראש \"כל הארץ\" מוצג סיכום של סך-הק\"מ שייחסך בכל הארץ — ליום ולשבוע — אם כל העיקופים האמיתיים יתוקנו. אפשר למיין לפי הבזבוז.",
    "אפשר לעצור באמצע גם את *איתור-העיר* וגם את *עיבוד הקובץ*: לחיצה על ✕ (או על כפתור הביטול) בזמן חיפוש/סריקה ארוכים עוצרת מיד ומחזירה את החלון — בלי לחכות שייגמר.",
    "שיפורים בגלאי \"סיבוב/לולאה מיותרים\" לצמצום תקלות-שווא: מסלול שחוצה את עצמו לא נמדד יותר כאורך הלולאה כולה, וקו-השוואה משמש כהוכחה רק כשהוא מגיע לקטע מאותו כיוון כמו הקו הנבדק. לולאות אמיתיות (כולל הצגת קו-ההשוואה הירוק) נשמרות.",
    "מספור הגרסאות עבר למבנה דו-ספרתי פשוט (גדולה.קטנה). ההיסטוריה הישנה ביומן נשמרה כפי שהייתה.",
  ] },
      ...
      <div className="modal country-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {data && (
          <>
            ...
            {data.totalWasteDayKm != null && (
              <div className="country-savings">
                <span className="cs-ico">🚌</span>
                <span className="cs-text">
                  תיקון כל העיקופים האמיתיים יחסוך בסך הכל{" "}
                  <b>{Number(data.totalWasteDayKm).toLocaleString("he-IL")} ק"מ ביום עמוס</b>
                  {" — כ-"}
                  <b>{Number(Math.round(data.totalWasteDayKm * 6)).toLocaleString("he-IL")} ק"מ בשבוע</b>
                  <span className="cs-note"> (אומדן לפי ~6 ימי-פעילות בשבוע)</span>
                </span>
              </div>
            )}

...
const CHANGELOG = [
  { version: "1.1.3", date: "10.6.2026", items: [
    "זיהוי הקפת-כיכר / לולאה-סגורה: כשמסלול הקו מקיף וחוזר לאותה נקודה (בעוד קו אחר עובר ישר) — מסומן כעת \"עיקוף אמיתי\". מבדיל לולאה-סגורה אמיתית מהתעקלות-כביש פתוחה לגיטימית.",
  ] },
...
// מרחק ה"חזרה-העצמית" המינימלי (ק"מ): המרחק הקטן ביותר בין שתי נקודות במסלול
// שביניהן אורך-מסלול ≥ minAlongKm. ערך זעיר = המסלול חוזר לנקודה שכבר ביקר בה
// (לולאה סגורה — הקפת כיכר / טיפה), בניגוד להתעקלות-כביש פתוחה שלעולם אינה חוזרת.
function pathSelfReturnKm(pts, minAlongKm) {
  if (!pts || pts.length < 8) return Infinity;
  const lat0 = pts[0][0], lng0 = pts[0][1];
  const kx = 111.32 * Math.cos(lat0 * Math.PI / 180), ky = 110.57;
  const xy = pts.map((p) => [(p[1] - lng0) * kx, (p[0] - lat0) * ky]);
  const cum = [0];
  for (let i = 1; i < xy.length; i++) cum[i] = cum[i - 1] + Math.hypot(xy[i][0] - xy[i - 1][0], xy[i][1] - xy[i - 1][1]);
  let best = Infinity;
  for (let i = 0; i < xy.length; i++) for (let j = i + 6; j < xy.length; j++) {
    if (cum[j] - cum[i] < minAlongKm) continue;
    const dd = Math.hypot(xy[i][0] - xy[j][0], xy[i][1] - xy[j][1]);
    if (dd < best) best = dd;
  }
  return best;
}
...
function geoClassify(d) {
  ...
  // (1ב) לולאה סגורה — המסלול חוזר לכמעט אותה נקודה אחרי הקפה (≥60 מ' מסלול ביניהן):
  //      הקפת כיכר מלאה / טיפה, בעוד קיים נתיב ישר (יחס > 1). זה מבדיל לולאה-סגורה
  //      (חוזרת לעצמה) מהתעקלות-כביש פתוחה שלעולם אינה חוזרת. כמו קו 70 שמקיף את כל
  //      הכיכר בשד' בן גוריון, בעוד קווים אחרים עוברים ישר. דורש עודף-מרחק ממשי.
  const CLOSED_LOOP_KM = 0.020; // 20 מ' — חזרה לאותה נקודה
  if (ratio >= 1.2 && pathSelfReturnKm(d.lineGeometry, 0.060) <= CLOSED_LOOP_KM) {
    return { verdict: "אמיתי", reason: `המסלול מקיף וחוזר לאותה נקודה (לולאה סגורה — הקפת כיכר/טיפה) ואז ממשיך, בעוד קו ${d.refNumber} עובר ישר (יחס פי ${ratio.toFixed(2)}) — הקפה מיותרת, עיקוף אמיתי.`, fallback: true };
  }

...
// "מוצא<->יעד-מק"ט#" (route_long_name) → "מוצא ← יעד" קריא
function fmtDir(d) {
  if (!d) return "";
  const parts = String(d).split("<->");
  if (parts.length !== 2) return d;
  const clean = (x) => x.replace(/-\d+[א-ת#]?#?$/, "").trim();
  return clean(parts[0]) + " ← " + clean(parts[1]);
}
// "10388-1-1" (route_desc: מק"ט-כיוון-חלופה) → "כיוון 1 · חלופה 1 · מק״ט 10388"
function fmtRd(rd) {
  if (!rd) return "";
  const p = String(rd).split("-");
  if (p.length < 3) return rd;
  // חלופה מוצגת כמו שהיא ברישוי — '#' או '0' הן החלופה הראשית (בקשת משתמש:
  // להציג את הסימן עצמו, פחות טקסט)
  return `כיוון ${p[1]} · חלופה ${p[2]}`;
}
...
const CHANGELOG = [
  { version: "5.1", date: "6.8.2026", items: [
    "הקטע הכתום מדויק יותר (בקשת משתמש): לולאות והלוך-חזור מסומנים עכשיו בשלמותם — מנקודת העזיבה של המסלול הסביר ועד החזרה אליו, כולל נקודת-הפנייה (קו 547, קו 283) — במקום רק הנקודות שכבר צברו סטייה גדולה.",
    "ובכיוון השני: הכתום לא נמתח יותר מאות מטרים על דרך תקינה כשנקודות הקו דלילות (קו 46 טירת הכרמל) — קצה הסימון נחתך בנקודה שבה הקו באמת מתרחק מהמסלול הסביר.",
  ] },
  { version: "5.0", date: "6.8.2026", items: [
    "לחיצה על \"הצג על המפה\" ממקדת עכשיו את המפה בתקלה עצמה (המקטע הכתום וקו-ההשוואה) במקום בכל מסלול הקו (בקשת משתמש). הקו המלא עדיין מצויר — אפשר להתרחק ידנית.",
    "בשורת המקטע ובפאנל: קודם כיוון הקו וספרת החלופה, ורק אחר-כך מאיפה-לאיפה — בלי צורך לגלול (בקשת משתמש). החלופה מוצגת כמו שהיא ברישוי ('#' או '0' = החלופה הראשית).",
    "נוספה העיר של המקטע השגוי (📍) ליד שם המקטע — בטבלה ובפאנל. נשלפת מנתוני התחנות של משרד התחבורה; תופיע החל מהסריקה היומית הבאה.",
    "הסריקה היומית אוחדה לצינור אחד עם שאר אתרי הקו הבוחן: קובץ ה-GTFS יורד פעם אחת ומשמש את כולם, והתוצאות מסונכרנות לכאן אוטומטית.",
  ] },
  ...
  { version: "4.6", date: "6.7.2026", items: [
    ...
    "תוקן דיווח-שווא נוסף מאותה משפחת-תקלות (הצמדה למסלול חוצה-עצמו, ראו גרסה 3.17): קווים 34/36 (\"דליה\") וקו 97 (סביבת \"זמיר\") דיווחו על עיקוף גדול שלא היה קיים בפועל, כי נקודת-המדידה נתקעה על חצייה שגויה של המסלול. נוסף מבחן ישיר שמשווה את נקודת-המדידה למיקום האמיתי של התחנה — מזהה את התקלה גם כשהבדיקה הקודמת (מרחק לאורך המסלול בלבד) פספסה אותה, בלי לפסול בטעות מקטעים תקינים שנראים דומים במבט ראשון.",
    "נוספה קטגוריית-הכרעה חדשה: \"חשד עיקוף (רק לפי רכב)\" — מקטעים שאין להם אף קו-אוטובוס אחר שמוכיח דרך קצרה יותר (ולכן הגלאי הרגיל לא רואה אותם בכלל), אבל המרחק המצויר גדול משמעותית מהקו-האווירי בין שתי התחנות. נבדקים בנפרד מול ניווט-רכב אובייקטיבי, ומאושרים רק אם גם הניווט מראה פער גדול. זו קטגוריה נפרדת מ\"אמיתי\" — פחות ודאית, כי אין כאן הוכחת-קו, רק פער-גיאומטרי ואישור-רכב בודד.",
    "תוקן: במקטעי \"חשד עיקוף (רק לפי רכב)\" שיש בהם לולאה (למשל כניסה למתחם), הקטע הכתום סימן רק את הכניסה והיציאה מהלולאה ולא אותה עצמה — כי בלי קו-ייחוס ירוק, הזיהוי נפל לבדיקה חלשה יותר (נסיגה על ציר ישר בין שתי התחנות, שלא תופסת לולאה שלמה). עכשיו משתמשים בדרך-הקצרה-בכביש (הסגול) כרפרנס במקומו, וכל הלולאה מסומנת.",
    "הרחבת-אזהרה בהסבר של \"חשד עיקוף (רק לפי רכב)\": יתכן שהפער מוסבר על-ידי מסדרון-תח\"צ ייעודי (כמו נתיב-BRT/מטרונית) שרכב פרטי לא יכול לנסוע בו — ה-OSRM לא רואה נתיבים כאלה, כך שהמרחק-האופטימלי-לרכב שהוא מחשב אינו רלוונטי במקרה כזה.",
    "מקטעי \"חשד עיקוף (רק לפי רכב)\" שצורתם חוזרת כמעט לאותה נקודה שממנה יצאו לפני שממשיכים הלאה (סימן לגישה לתחנת-קצה בדרך-ללא-מוצא, כמו בית-חולים) הוסרו מהקטגוריה — בבדיקה התברר שזה חל על רוב המקטעים (כ-70%!), כנראה גישה לגיטימית ולא עיקוף-צד. הקטגוריה ירדה מ-445 ל-137 רשומות.",
    "תוקן: כשה-OSRM לא באמת מסיים ליד התחנה (למשל תחנת-מוצא/מסוף עם גישה פרטית שהוא לא רואה, ומצמיד לכביש ציבורי קרוב אבל שונה) — המקטע לא מאושר, כי ההשוואה לא אמינה. הוריד את \"חשד עיקוף (רק לפי רכב)\" מ-137 ל-110 רשומות.",
    "תוקן זיהוי-רטרייס חלקי: הבדיקה הקודמת (התקדמות על ציר מוצא-יעד) פספסה מקטעים שהלוך-חזור בהם מתלכד בכיוונו עם היעד הסופי (למשל קו 45 \"עלמה 1\"←\"עלמה 2\" — 27% מהמקטע חוזר על עצמו, לא זוהה). נוסף מבחן ישיר על הגאומטריה עצמה (נקודה קרובה לנקודה מוקדמת יותר באותו מקטע), בלי תלות בכיוון. הוריד את \"חשד עיקוף (רק לפי רכב)\" מ-110 ל-28 רשומות — רוב מה שנשאר קודם התברר כאותה תקלה בדיוק.",
  ] },

...
const CHANGELOG = [
  { version: "4.9", date: "4.8.2026", items: [
    ...
    "לכל ממצא מוצג עכשיו גם כיוון-הנסיעה של הקו (🧭 מוצא ← יעד) — בטבלה ובפאנל — כדי שיהיה ברור לאיזה כיוון של הקו שייך המקטע (בקשת משתמש). הנתון נכנס לדוח מהסריקה הבאה ואילך.",
  ] },
                  ...
                  {shown.map((i, k) => {
                    ...
                    return (
                      <tr key={k} className={onPick && hasGeo ? "clickable" : ""}
                        ...
                        <td>{i.operator}</td>
                        <td className="seg">{i.from} → {i.to}{onPick && hasGeo ? <span className="map-ico"> 🗺️</span> : null}{i.dir ? <div className="seg-dir" title="כיוון הנסיעה של הקו שבו נמצא המקטע">🧭 {fmtDir(i.dir)}</div> : null}</td>
